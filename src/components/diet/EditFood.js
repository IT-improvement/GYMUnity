...
const EditFood = () => {
  ...
  const fetchFoodUpdate = () => {
    ...
    console.log("foodCategoryIndex:" + food.categoryIndex);
    console.log("foodName:" + food.name);
    console.log("protein:" + food.protein);
    console.log("calory:" + food.calory);
    console.log("carbs:" + food.carbs);
    console.log("size:" + food.size);
    const requestBody = {
    ...
    fetch(
      `${process.env.REACT_APP_SERVER_URL}/food/service?command=updateFood`,
      {
        method: "POST",
        headers: {
      ...
      .finally(() => {
        ...
        if (isUpdated) navigate(`/diet/viewFoodDetail/${foodIndex}`);
      });
  };
  ...
  return (
    <Flex w="100%" justify="center">
      <form
        style={{ minWidth: "70%" }}
        method="POST"
        onSubmit={handleFoodUpdateOnSubmit}
      >
        <FormControl>
