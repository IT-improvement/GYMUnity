...
export default function RoutineDiet() {
    ...
    useEffect(() => {
        fetch(`${process.env.REACT_APP_SERVER_URL}/routineDiet?command=read`)
            ...
            .then((data) => {
                setBreakfast(data.filter((item) => item.category === '아침'));
                setLunch(data.filter((item) => item.category === '점심'));
                setDinner(data.filter((item) => item.category === '저녁'));
                console.log(data);
            })
            .catch((error) => {
    ...
    const onClickDeleteButton = () => {
        if (status) {
            if (selectedItems.length > 0) {
                fetch(`${process.env.REACT_APP_SERVER_URL}/routineDiet?command=delete`, {
                    method: 'POST',
                    headers: {
